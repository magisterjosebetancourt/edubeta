...
import { Trash2, Mail, Plus, Pencil, Copy, CheckCircle2 } from "lucide-react";
    ...
    <div className="bg-background-light dark:bg-background-dark min-h-screen pb-24">
      <div className="p-4 lg:p-8 space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between px-1">
          ...
          <Button
            ...
            className="bg-primary hover:bg-primary/90 text-white rounded-lg h-auto py-3.5 px-6 gap-2 shadow-xl shadow-primary/20 font-semibold text-xs tracking-wide w-full sm:w-auto transition-all active:scale-95"
          >
      ...
      <main className="p-4 space-y-4">
        ...
        {invites.length > 0 && (
          <div className="bg-white dark:bg-[#151b2d] rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-4">
            ...
            <div className="space-y-2">
              {invites.map(invite => (
                <div key={invite.email} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/30 rounded-lg">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => handleDeleteInvite(invite.email)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded transition-all">
        ...
        <div className="bg-white dark:bg-[#151b2d] rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-4">
          ...
          <div className="space-y-2">
            {teachers.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-6">No hay docentes registrados aún.</p>
            ) : (
              teachers.map(teacher => (
                <div key={teacher.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/30 rounded-lg">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">{teacher.full_name || "Sin nombre"}</p>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <button onClick={() => handleDeleteTeacher(teacher.id)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
