...
export default function NewScheduleFormPage() {
  ...
  useEffect(() => {
    if (
      ...
      !availableSubjects.find((s) => s.subject_id === selectedSubjectId)
    ) {
    ...
    const loadAssignments = async () => {
      try {
        const [assSnap, gradSnap, subSnap] = await Promise.all([
          ...
          getDocs(
            query(collection(db, "subjects"), where("state", "==", true)),
          ),
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const assignment = assignments.find(
      (a) =>
        a.grade_id === selectedGroupId && a.subject_id === selectedSubjectId,
    );
    if (!assignment) {
      setSaving(false);
  ...
  return (
    <FormView>
      <p className="text-sm text-slate-500 dark:text-slate-400">
        Programa el horario del docente en la semana.
      </p>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {assignments.length === 0 && (
          <p className="text-[10px] text-amber-500 font-medium mt-1">
            El docente necesita tener asignación académica para poder
            programarle horario.
          </p>
        )}
