...
export default function SchedulesPage() {
  ...
  const activeTeacherId =
    userRole === "teacher" ? profile?.uid : selectedTeacherId;
  ...
  const todaysClasses = useMemo(() => {
    ...
    return classes.sort(
      (a: any, b: any) => Number(a.time_item) - Number(b.time_item),
    );
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("¿Seguro que deseas eliminar esta hora de clase?"))
      return;
    try {
      ...
      <div className="p-4 lg:p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          {activeTeacherId && (
            <EduButton
              onClick={() =>
                navigate(
                  `/dashboard/schedules/new${userRole !== "teacher" ? `?teacher_id=${activeTeacherId}` : ""}`,
                )
              }
              icon={Plus}
        ...
        {!activeTeacherId ? (
          ...
          <div className="space-y-6">
            ...
            <Card className="border-primary/20 bg-primary/5 dark:bg-primary/10 shadow-sm overflow-hidden relative">
              ...
              <CardHeader className="pl-6 pb-2">
                <CardTitle className="text-primary flex items-center gap-2 text-base">
                  ...
                  Clases de Hoy{" "}
                  {todayName ? `(${todayName})` : "(Fin de semana)"}
                </CardTitle>
                ...
                {!todayName || todaysClasses.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {todaysClasses.map((cls: any) => (
                      <div
                        key={cls.id}
                        className="bg-white dark:bg-[#151b2d] p-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-2"
                      >
                        ...
                        <div className="flex items-center gap-1 mt-auto pt-2 border-t border-slate-100 dark:border-slate-800">
                          <button
                            onClick={() =>
                              navigate(
                                `/dashboard/attendance/new?grade_id=${cls.group_id}&subject_id=${cls.subject_id}`,
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate(`/dashboard/schedules/${cls.id}/edit`)
                            }
                            title="Editar"
            ...
            {/* Weekly Schedule */}
            <div>
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4 px-1">
                Horario Semanal
              </h3>
                    ...
                    <div className="flex flex-col gap-2">
                      {weeklyClassesByDay[day].length === 0 ? (
                        ...
                        weeklyClassesByDay[day].map((cls: any) => (
                          <div
                            key={cls.id}
                            className="bg-white dark:bg-[#151b2d] p-3 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm hover:border-primary/30 transition-colors group relative"
                          >
                            ...
                            <div className="pl-1">
                              ...
                              <div className="flex items-center gap-0.5 mt-3 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                                ...
                                <button
                                  onClick={() =>
                                    navigate(
                                      `/dashboard/schedules/${cls.id}/edit`,
                                    )
                                  }
                                  title="Editar"
                                  className="p-1.5 text-slate-400 hover:text-primary hover:bg-primary/10 rounded transition-all"
