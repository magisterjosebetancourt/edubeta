      ...
      <main className="p-4 space-y-6">
        <Card className="border-none shadow-none bg-transparent">
          ...
          <CardContent className="px-0 space-y-6">
            {grades.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed border-slate-200 rounded-[5px] bg-white/50 dark:bg-slate-900/30">
              ...
              LEVELS.map((level) => {
                ...
                return (
                  <div
                    key={level.id}
                    className="bg-white dark:bg-[#151b2d] rounded-[5px] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm flex flex-col"
                  >
                    ...
                    <div className="p-4 flex flex-col gap-3">
                      {levelGroups.map((group) => (
                        <div
                          key={group.id}
                          className="relative group/item flex flex-col bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-[5px] shadow-sm hover:border-primary/50 transition-colors overflow-hidden"
                        >
                          <div className="flex items-center justify-between p-4">
                            <div className="flex flex-col">
                              ...
                              {group.director_id && (
                                <p className="text-[10px] text-slate-400 mt-0.5 italic flex items-center gap-1">
                          ...
                          <div className="px-3 pb-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-nowrap items-center justify-end w-full gap-1 overflow-x-auto bg-slate-50/50 dark:bg-black/10">
                            ...
                            >
                              {group.state !== false ? <UserMinus className="w-4 h-4" /> : <UserCheck className="w-4 h-4" />}
                              <span className="truncate">{group.state !== false ? "Desactivar" : "Activar"}</span>
                            </button>
                            <button
