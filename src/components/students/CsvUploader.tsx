    ...
    Papa.parse<CsvRow>(file, {
      ...
      complete: (results) => {
        if (results.errors.length > 0) {
          setErrors(
            results.errors.map((e: any) => `Fila ${e.row}: ${e.message}`),
          );
          return;
        }
  ...
  const parseText = (text: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (parts.length >= 3) {
        ...
        newErrors.push(
          `Fila ${index + 1}: Formato incorrecto. Debe tener al menos "apellido, nombre, grado"`,
        );
      }
    });
  ...
  const validateGrades = async (rows: CsvRow[]) => {
    ...
    try {
      ...
      uniqueCsvGrades.forEach((csvGradeName) => {
        const exists = initialGrades.some(
          (g) =>
            (g.name || "").toLowerCase() === (csvGradeName || "").toLowerCase(),
        );
        if (!exists) {
          newErrors.push(
            `El grado "${csvGradeName}" no existe en la base de datos.`,
          );
        }
      });
      ...
      for (const chunk of chunks) {
        ...
        chunk.forEach((row) => {
          const grade = initialGrades.find(
            (g) =>
              (g.name || "").toLowerCase() === row.grado.trim().toLowerCase(),
          );
    ...
    <div className="space-y-4">
      ...
      {mode === "csv" ? (
        ...
        <div className="space-y-2">
          <Label htmlFor="text-list">
            Lista de estudiantes (apellido, nombre, grado, barrio opcional)
          </Label>
          <Textarea
            id="text-list"
          ...
          <p className="text-[10px] text-slate-500 dark:text-slate-400">
            * Un estudiante por fila. Formato: apellido, nombre, grado, barrio
            (opcional).
          </p>
        </div>
      )}
